...
RegisterNetEvent('fivem-appearance:clothingShop', function()
	exports['qb-menu']:openMenu({
        {
            header = "👚 | Pilihan toko pakaian",
            isMenuHeader = true, -- Set to true to make a nonclickable title
        },
        {
            header = "Beli pakaian - $"..Config.Money,
			txt = "Pilih dari berbagai item untuk dipakai",
            params = {
                event = "fivem-appearance:clothingMenu",
            }
        },
		{
            header = "Berganti pakaian",
			txt = "Pilih dari salah satu pakaian yang Anda simpan saat ini",
            params = {
		...
		{
            header = "Simpan pakaian baru",
			txt = "Simpan pakaian baru yang bisa Anda gunakan nanti",
            params = {
                event = "fivem-appearance:saveOutfit",
            }
        },
		{
            header = "Hapus pakaian",
			txt = "Ya ... kami juga tidak suka yang itu",
            params = {
...
RegisterNetEvent('fivem-appearance:saveOutfit', function()
	...
	if keyboard ~= nil then
		...
		QBCore.Functions.Notify('Pakaian '..keyboard.input.. ' sudah disimpan', 'success')
	end
end)
...
RegisterNetEvent('fivem-appearance:deleteOutfitMenu', function(data)
    ...
    for i=1, #allMyOutfits, 1 do
        DeleteMenu[#DeleteMenu + 1] = {
            header = 'Menghapus "'..allMyOutfits[i].name..'"',
			txt = 'Anda tidak akan pernah bisa mendapatkan ini kembali!',
            params = {
...
RegisterNetEvent('qb-clothing:client:openOutfitMenu', function()  -- Name is so that you dont have to replace the event, Used in Appartments, Bossmenu, etc...
	exports['qb-menu']:openMenu({
		...
		{
            header = "Berganti pakaian",
			txt = "Pilih dari salah satu pakaian Anda saat ini yang disimpan",
            params = {
...
RegisterNetEvent('qb-clothing:client:openWorkOutfits', function(datawork)  -- Name is so that you dont have to replace the event, Used in Appartments, Bossmenu, etc...
	exports['qb-menu']:openMenu({
		...
		{
            header = "Pakaian sipil",
			txt = "Kenakan pakaian Anda yang biasa",
            params = {
                event = "fivem-appearance:reloadskin",
            }
        },
		{
            header = "Pakaian kerja",
			txt = "Pilih pakaian kerja",
            params = {
...
RegisterNetEvent('fivem-appearance:deleteOutfitMenuApp', function(data)
    ...
    for i=1, #allMyOutfits, 1 do
        DeleteMenu[#DeleteMenu + 1] = {
			...
			txt = 'You will never be able to get it back!',
            params = {
				event = 'fivem-appearance:deleteOutfit',
				args = allMyOutfits[i].id
			}
...
CreateThread(function()
    ...
    --             exports['qb-core']:DrawText('[E] Clothing Store', 'left')
    ...
    clothingRoomsCombo:onPlayerInOut(function(isPointInside, point, zone)
        if isPointInside then
            ...
            if (PlayerData.job.name == Config.ClothingRooms[tonumber(string.sub(zone.name, 15))].requiredJob) then
                inZone = true
                exports['qb-core']:DrawText('[E] Dressing Room', 'left')
            end
        else
            inZone = false
            exports['qb-core']:HideText()
        ...
        if inZone then
            ...
            if string.find(zoneName, 'ClothingRooms_') then
                if IsControlJustReleased(0, 38) then
                    ...
                    TriggerEvent("qb-clothing:client:openWorkOutfits", Config.Outfits[PlayerJob.name][gender][PlayerData.job.grade.level])
                end
